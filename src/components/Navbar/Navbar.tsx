...
const Navbar = () => {
    ...
    useEffect(() => {
        ...
        if(navbarTogglerRef.current) mainNavRef.current.classList.add('d-none');

        const navbarTogglerListener = (ev: Event) => {
            mainNavRef.current && mainNavRef.current.classList.toggle('d-none');
        }

        // This makes the nav-menu visible or invisible
        navbarTogglerRef.current.addEventListener('click', navbarTogglerListener)

        // Listener function for the nav-menu
        const mainNavListener = (ev: Event) => {
            mainNavRef.current && mainNavRef.current.classList.add('d-none');
        }
